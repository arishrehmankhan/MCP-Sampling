...
## Overview
This server demonstrates MCP's **sampling**, **roots**, and **resources** features, allowing clients to request LLM completions, workspace information, and access structured data without server-side API keys.
...
### 1. server-info (mcp://server/info)
Static resource providing server metadata and capabilities.
- **Use when**: Need information about the server
- **Returns**: Server name, version, capabilities, tool/resource counts

### 2. sample-data (mcp://samples/{id})
Dynamic resource with template parameters serving example data by ID.
- **Use when**: Need example data for testing or demonstration
- **Available IDs**: 1, 2, 3
- **Returns**: Sample data with title, content, category, and tags

### 3. text-snippets (mcp://snippets/{category})
Pre-defined text snippets for analysis and demonstration.
- **Use when**: Need sample text for testing tools
- **Available categories**: positive, negative, technical, story
- **Returns**: Text snippet with title

### 4. analyzed-texts (mcp://analyzed/{textId})
Resources that combine with sampling to provide AI-enhanced data access.
- **Use when**: Need pre-analyzed text samples
- **Available IDs**: review1, review2, quote1, quote2
- **Returns**: Original text with AI analysis and timestamp
...
1. Client requests resource by URI
2. Server retrieves or generates data
3. Server returns structured content
4. No state changes or side effects occur

**Benefit**: Application-controlled data access with clear separation from tools.
...
2. **Resource Access**: Use resources for read-only data, tools for actions
3. **Elicitation**: When data is incomplete, let elicitation tools gather it interactively
4. **Multi-step Workflows**: Use wizard tools for complex, sequential data gathering
5. **Workspace Awareness**: Use analyze-workspace first to understand available directories
6. **Model Preferences**: The server suggests appropriate models based on task complexity
...
async function main() {
    ...
    console.error('  - Instructions: Clients can request usage guidance via instructions/list');
}
